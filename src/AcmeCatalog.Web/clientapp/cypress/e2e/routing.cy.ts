...
// still deliberately served by Razor: /Items/ImagePreview/{id}, the iframe
// src behind Quick View. The old *.cy.js suite deleted during the React
// migration only ever exercised Razor views at these URLs — this confirms
// every one of them now resolves to the intended React page instead.
...
describe('Client-side routing', () => {
  ...
  it('resolves every top-level route to its React page', { tags: '@smoke' }, () => {
    ...
    cy.request('/api/items').its('body.0.id').then((itemId) => {
  ...
  it('/Items/ImagePreview/{id} still serves real server-rendered HTML', { tags: '@smoke' }, () => {
    cy.request('/api/items').its('body.0').then((item) => {
      cy.visit(`/Items/ImagePreview/${item.id}`)
